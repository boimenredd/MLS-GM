...
function percentileFromFbrefMetric(value, average, spread = 1) {
  const safeSpread = Math.max(0.08, spread || 1);
  const z = (Number(value || 0) - Number(average || 0)) / safeSpread;
  const pct = 50 + (z * 18);
  return clamp(Math.round(pct), 18, 92);
}

function normalizedFbrefPos(raw) {
  const txt = String(raw || '').toUpperCase();
  if (txt.includes('GK')) return 'GK';
  if (txt.includes('FW') && txt.includes('MF')) return 'AM/W';
  if (txt.includes('FW')) return 'ST';
  if (txt.includes('DF') && txt.includes('MF')) return 'FB/DM';
  if (txt.includes('DF')) return 'CB';
  if (txt.includes('MF')) return 'CM';
  return 'CM';
}

function inferSpecificPositionFromFbref(row) {
  const raw = normalizedFbrefPos(row?.fbrefPos || row?.position || 'CM');
  const shot = Number(row?.fbrefShots || 0);
  const xg = Number(row?.fbrefXg || 0);
  const ast = Number(row?.fbrefAssists || 0);
  const xag = Number(row?.fbrefXag || 0);
  const kp = Number(row?.fbrefKeyPasses || 0);
  const prgc = Number(row?.fbrefProgCarries || 0);
  const prgp = Number(row?.fbrefProgPasses || 0);
  const tkl = Number(row?.fbrefTackles || 0);
  const intl = Number(row?.fbrefInterceptions || 0);
  const aerial = Number(row?.fbrefAerialWon || 0);
  const foot = String(row?.preferredFoot || 'Right').toLowerCase();
  if (raw === 'GK') return 'GK';
  if (raw === 'ST') return (ast + xag + kp >= xg + shot * 0.12) ? (foot === 'left' ? 'RW' : 'LW') : 'ST';
  if (raw === 'AM/W') {
    if (xg + shot * 0.15 > ast + xag + kp * 0.35) return foot === 'left' ? 'RW' : 'LW';
    return 'CAM';
  }
  if (raw === 'FB/DM') {
    if (tkl + intl > prgc + kp + xag * 2.4) return 'CDM';
    return foot === 'left' ? 'LB' : 'RB';
  }
  if (raw === 'CB') {
    if (prgc + kp + xag * 2.2 > aerial + tkl + intl) return foot === 'left' ? 'LB' : 'RB';
    return 'CB';
  }
  // CM bucket
  if (tkl + intl > kp + xag * 3.0) return 'CDM';
  if (kp + xag * 3.0 > tkl + intl + xg * 2.0) return 'CAM';
  return 'CM';
}

function buildRealProfileFromFbrefRow(row, requestedPosition) {
  const position = requestedPosition || inferSpecificPositionFromFbref(row);
  const minutes = Number(row?.fbrefMinutes || 0);
  const sampleBoost = minutes >= 1800 ? 1 : minutes >= 900 ? 0.92 : minutes >= 450 ? 0.84 : 0.75;

  const gk = position === 'GK';
  const goal = Number(row?.fbrefGoals || 0);
  const ast = Number(row?.fbrefAssists || 0);
  const xg = Number(row?.fbrefXg || 0);
  const xag = Number(row?.fbrefXag || 0);
  const shots = Number(row?.fbrefShots || 0);
  const sot = Number(row?.fbrefSot || 0);
  const kp = Number(row?.fbrefKeyPasses || 0);
  const prgp = Number(row?.fbrefProgPasses || 0);
  const prgc = Number(row?.fbrefProgCarries || 0);
  const tkl = Number(row?.fbrefTackles || 0);
  const itc = Number(row?.fbrefInterceptions || 0);
  const blk = Number(row?.fbrefBlocks || 0);
  const aerial = Number(row?.fbrefAerialWon || 0);
  const savePct = Number(row?.fbrefSavePct || 0);
  const ga90 = Number(row?.fbrefGa90 || 0);

  const heightText = String(row?.height || '');
  const heightCmMatch = heightText.match(/(\d+)cm/);
  const heightCm = heightCmMatch ? Number(heightCmMatch[1]) : 180;
  const age = Number(row?.age || 25);

  const physical = {
    Height: clamp(Math.round((heightCm - 150) * 0.9), 35, 92),
    Strength: percentileFromFbrefMetric(aerial + blk * 0.4, 2.0, 1.2),
    SprintSpeed: percentileFromFbrefMetric(prgc, 3.0, 1.8),
    Acceleration: percentileFromFbrefMetric(prgc + kp * 0.25, 3.6, 2.0),
    Endurance: clamp(percentileFromFbrefMetric((minutes / 90) / 18, 1.0, 0.45), 22, 92),
  };

  const passing = {
    Vision: percentileFromFbrefMetric(kp + xag * 2.0, 1.3, 0.95),
    Power: percentileFromFbrefMetric(prgp, 4.8, 2.2),
    Accuracy: percentileFromFbrefMetric(prgp + kp * 0.5 - blk * 0.08, 5.3, 2.1),
    Crossing: gk ? 8 : percentileFromFbrefMetric(xag + kp * 0.3 + prgc * 0.2, 1.1, 0.8),
    LongPassing: percentileFromFbrefMetric(prgp, 5.0, 2.3),
  };

  const shooting = {
    ShotPower: gk ? 8 : percentileFromFbrefMetric(shots + xg * 2.0, 2.2, 1.6),
    HeadingAccuracy: gk ? 12 : percentileFromFbrefMetric(aerial + goal * 0.3, 2.4, 1.3),
    Volleys: gk ? 6 : percentileFromFbrefMetric(goal + sot * 0.35, 0.7, 0.7),
    FreeKickAccuracy: gk ? 5 : percentileFromFbrefMetric(kp + xag, 1.0, 0.7),
    Curve: gk ? 5 : percentileFromFbrefMetric(xag + kp * 0.35, 1.1, 0.75),
  };

  const skill = {
    Dribbling: gk ? 10 : percentileFromFbrefMetric(prgc + shots * 0.25, 4.0, 2.0),
    BallControl: gk ? 16 : percentileFromFbrefMetric(prgc + prgp * 0.25, 5.2, 2.3),
    SkillMoves: gk ? 4 : percentileFromFbrefMetric(prgc, 3.2, 1.7),
  };

  const mentality = {
    Aggression: percentileFromFbrefMetric(tkl + blk * 0.5, 2.5, 1.5),
    Positioning: gk ? percentileFromFbrefMetric(savePct - ga90 * 5, 45, 10) : percentileFromFbrefMetric(xg + kp * 0.3 + itc * 0.18, 1.5, 0.9),
    Penalties: gk ? 6 : percentileFromFbrefMetric(goal + xg * 0.8, 0.8, 0.8),
    Composure: percentileFromFbrefMetric(kp + prgp * 0.22 + prgc * 0.16, 2.1, 1.0),
  };

  const defense = {
    Awareness: gk ? 18 : percentileFromFbrefMetric(itc + blk * 0.55, 2.2, 1.1),
    StandingTackle: gk ? 12 : percentileFromFbrefMetric(tkl, 1.8, 1.0),
    SlidingTackle: gk ? 8 : percentileFromFbrefMetric(tkl * 0.8 + blk * 0.2, 1.5, 0.9),
    Interceptions: gk ? 12 : percentileFromFbrefMetric(itc, 1.2, 0.8),
  };

  const goalkeeping = {
    Diving: gk ? clamp(percentileFromFbrefMetric(savePct, 68, 9), 35, 92) : 5,
    Handling: gk ? clamp(percentileFromFbrefMetric(savePct - ga90 * 3, 66, 9), 34, 92) : 5,
    Kicking: gk ? clamp(percentileFromFbrefMetric(prgp, 4.0, 2.2), 28, 88) : 5,
    Positioning: gk ? clamp(percentileFromFbrefMetric(80 - ga90 * 10 + savePct * 0.2, 70, 9), 34, 92) : 5,
    Reflexes: gk ? clamp(percentileFromFbrefMetric(savePct + shots * 0.3, 70, 10), 35, 94) : 5,
  };

  const categoryRatings = {
    physical: categoryAverage(physical),
    passing: categoryAverage(passing),
    shooting: categoryAverage(shooting),
    skill: categoryAverage(skill),
    mentality: categoryAverage(mentality),
    defense: categoryAverage(defense),
    goalkeeping: categoryAverage(goalkeeping),
  };

  const overallBase = Object.entries(positionWeights(position)).reduce((sum, [key, wt]) => sum + (categoryRatings[key] || 0) * wt, 0);
  const overallRating = clamp(Math.round((overallBase * sampleBoost) + ageRealRatingModifier(age, position)), 42, 88);

  const detailed = {
    physical: {
      acceleration: clamp(Math.round(physical.Acceleration), 5, 99),
      sprintSpeed: clamp(Math.round(physical.SprintSpeed), 5, 99),
      agility: clamp(Math.round((skill.Dribbling + skill.BallControl) / 2), 5, 99),
      stamina: clamp(Math.round(physical.Endurance), 5, 99),
      strength: clamp(Math.round(physical.Strength), 5, 99),
      jumping: clamp(Math.round((physical.Height + shooting.HeadingAccuracy) / 2), 5, 99),
    },
    technical: {
      finishing: clamp(Math.round((shooting.ShotPower + mentality.Positioning + shooting.Volleys) / 3), 5, 99),
      longShots: clamp(Math.round((shooting.ShotPower + shooting.Curve) / 2), 5, 99),
      crossing: clamp(Math.round(passing.Crossing), 5, 99),
      shortPassing: clamp(Math.round(passing.Accuracy), 5, 99),
      vision: clamp(Math.round(passing.Vision), 5, 99),
      dribbling: clamp(Math.round(skill.Dribbling), 5, 99),
      firstTouch: clamp(Math.round(skill.BallControl), 5, 99),
      setPieces: clamp(Math.round((shooting.FreeKickAccuracy + shooting.Curve) / 2), 5, 99),
    },
    defending: {
      marking: clamp(Math.round(defense.Awareness), 5, 99),
      tackling: clamp(Math.round((defense.StandingTackle + defense.SlidingTackle) / 2), 5, 99),
      interceptions: clamp(Math.round(defense.Interceptions), 5, 99),
      heading: clamp(Math.round(shooting.HeadingAccuracy), 5, 99),
      positioning: clamp(Math.round(mentality.Positioning), 5, 99),
    },
    goalkeeping: {
      handling: clamp(Math.round(goalkeeping.Handling), 1, 99),
      reflexes: clamp(Math.round(goalkeeping.Reflexes), 1, 99),
      oneOnOnes: clamp(Math.round((goalkeeping.Diving + goalkeeping.Reflexes) / 2), 1, 99),
      kicking: clamp(Math.round(goalkeeping.Kicking), 1, 99),
      command: clamp(Math.round(goalkeeping.Positioning), 1, 99),
    },
  };

  const attributes = gk
    ? {
        pace: clamp(Math.round((physical.Acceleration + physical.SprintSpeed) / 2 * 0.75), 20, 82),
        shooting: clamp(Math.round(categoryRatings.shooting * 0.18), 5, 18),
        passing: clamp(Math.round((categoryRatings.passing + goalkeeping.Kicking) / 2), 20, 86),
        dribbling: clamp(Math.round(categoryRatings.skill * 0.45), 10, 48),
        defense: clamp(Math.round(categoryRatings.goalkeeping), 40, 95),
        physical: clamp(Math.round((categoryRatings.physical + goalkeeping.Positioning) / 2), 35, 92),
      }
    : {
        pace: clamp(Math.round((physical.Acceleration + physical.SprintSpeed) / 2), 20, 98),
        shooting: clamp(Math.round(categoryRatings.shooting), 20, 96),
        passing: clamp(Math.round(categoryRatings.passing), 20, 96),
        dribbling: clamp(Math.round(categoryRatings.skill), 20, 96),
        defense: clamp(Math.round(categoryRatings.defense), 20, 96),
        physical: clamp(Math.round(categoryRatings.physical), 20, 96),
      };

  return { detailed, categoryRatings, overallRating, attributes, inferredPosition: position };
}
...
function computeRealProfileFromRow(row, position) {
  if (row?.fbrefDerived || Number(row?.fbrefMinutes || 0) > 0) {
    const fbrefProfile = buildRealProfileFromFbrefRow(row, position || inferSpecificPositionFromFbref(row));
    if (fbrefProfile) return fbrefProfile;
  }
  if (row?.attributes && row?.detailed && Number(row?.overallRating || row?.overall || 0) > 0) {
    return {
      detailed: row.detailed,
      categoryRatings: row.categoryRatings || {},
      overallRating: Number(row.overallRating || row.overall || 0),
...
function makeRealMlsPlayer(team, row, idx, seasonYear = 2026) {
  ...
  const position = normalizeGeneratedPosition(row.position || row.inferredPosition || inferSpecificPositionFromFbref(row) || "CM", row.preferredFoot || "Right");
